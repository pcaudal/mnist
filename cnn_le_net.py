...
"""Architecture LeNet pour Convolutional Neural Networks for MNIST."""
...
#####     nb_filter= 30 (nombre de filtres de convolution)
...
model.add(Conv2D(filters=30, kernel_size=(5, 5), border_mode='valid', input_shape=(1, 28, 28), activation='relu', data_format="channels_first"))
...
##### Ajouter un deuxième ensemble de layer de convolution, Convolution => Activation: RELU => Pooling
#####     nb_filter= 15 (nombre de filtres de convolution)
#####     nb_row= 3 , nb_col= 3, (chancun des filtres à une taille de 5x5)
#####     border_mode = 'valid" (pour que les filtres ne s'appliquent qu'au
#####                            partie entières de l'image, ne dépassent pas les bords)
#####     input_shape= (1,28,28) (taille de chaque image)
#####     activation = 'relu'
...
#####                              aux channels de l'image (par défaut Conv2D considère
#####                              qu'il s'agit de la derniere coordonnée)
model.add(Conv2D(filters=15, kernel_size=(3, 3), border_mode='valid', input_shape=(1, 28, 28), activation='relu', data_format="channels_first"))

##### Puis ajouter un deuxième layer de pooling qui renvoie le maximum appelé MaxPooling2D,
##### que l'on configure avec un pool_size de taille (2,2).
model.add(MaxPooling2D(pool_size = (2,2)))
...
##### Finir par le dernier ensemble de layers: Fully-Connected => Fully-Connected => Activation: Softmax
#####     −>−> Les layer fully-connected renvoient des vecteurs de 50 puis 10 outputs,
#####          auxquels on applique la fonction d'activation "softmax" qui retournera
#####          une liste de probabilités, une pour chacun des 10 chiffres. Le chiffre avec
#####          la plus grande probabilité sera désigné comme classification finale.
...
model.fit(x = X_train , y = y_train, batch_size = 200 , epochs = 16 , verbose = 2)
